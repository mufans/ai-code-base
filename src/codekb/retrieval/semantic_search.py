...
class SemanticSearch:
    ...
    async def search_code(
        ...
        return self.vector_store.search_code(query_embedding, repo_name=repo_name,
    ...
    async def search_docs(
        ...
        query_embedding = await self.provider.embed_query(query)
        return self.vector_store.search_docs(query_embedding, repo_name=repo_name,
                                              repo_module=repo_module, top_k=top_k)
    ...
    async def search_all(
        ...
        query_embedding = await self.provider.embed_query(query)
        code_results = self.vector_store.search_code(query_embedding, repo_name=repo_name,
                                                      repo_module=repo_module, top_k=top_k)
        doc_results = self.vector_store.search_docs(query_embedding, repo_name=repo_name,
                                                     repo_module=repo_module, top_k=top_k)
