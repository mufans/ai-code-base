...
class DocStore:
    ...
    def _skills_dir(self, repo_name: str) -> Path:
        d = self._repo_dir(repo_name) / "skills"
        d.mkdir(parents=True, exist_ok=True)
        return d

    def write_doc(self, repo_name: str, filename: str, content: str):
        """Write a generated doc file."""
        path = self._repo_dir(repo_name) / filename
        path.write_text(content, encoding="utf-8")

    def read_doc(self, repo_name: str, filename: str) -> Optional[str]:
    ...
    def delete_doc(self, repo_name: str, filename: str) -> bool:
    ...
    def list_docs(self, repo_name: str) -> list[str]:
    ...
    def write_coverage(self, repo_name: str, coverage: dict):
        """Write COVERAGE.json for a repo."""
        path = self._repo_dir(repo_name) / "COVERAGE.json"
        path.write_text(json.dumps(coverage, indent=2, ensure_ascii=False), encoding="utf-8")

    def read_coverage(self, repo_name: str) -> Optional[dict]:
    ...
    def write_skill(self, repo_name: str, skill_name: str, content: str):
        """Write a skill file."""
        path = self._skills_dir(repo_name) / f"{skill_name}.md"
        path.write_text(content, encoding="utf-8")

    def read_skill(self, repo_name: str, skill_name: str) -> Optional[str]:
        """Read a skill file."""
        path = self._skills_dir(repo_name) / f"{skill_name}.md"
        if not path.exists():
            return None
    ...
    def list_skills(self, repo_name: str) -> list[str]:
        """List all skill files for a repo (without .md extension)."""
        skills_dir = self._skills_dir(repo_name)
        return [f.stem for f in skills_dir.iterdir() if f.is_file() and f.suffix == ".md"]

    def delete_skill(self, repo_name: str, skill_name: str) -> bool:
        """Delete a skill file."""
        path = self._skills_dir(repo_name) / f"{skill_name}.md"
        if path.exists():
            path.unlink()
